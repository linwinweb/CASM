...
CFLAGS="-m32 -Wall -O -fno-pie -fstrength-reduce -fomit-frame-pointer \
        ...
        -I./casm/include -I./misc/include                         \
        -I./tinybasic/include -c"
mkdir -p output &&                                                    \
cd src &&                                                             \
nasm -f elf32 -o start.o       casm/start.asm &&                  \
gcc ${CFLAGS} -o main.o        casm/main.c &&                     \
gcc ${CFLAGS} -o scrn.o        casm/scrn.c &&                     \
gcc ${CFLAGS} -o gdt.o         casm/gdt.c &&                      \
gcc ${CFLAGS} -o idt.o         casm/idt.c &&                      \
gcc ${CFLAGS} -o isrs.o        casm/isrs.c &&                     \
gcc ${CFLAGS} -o irq.o         casm/irq.c &&                      \
gcc ${CFLAGS} -o timer.o       casm/timer.c &&                    \
gcc ${CFLAGS} -o kb.o          casm/kb.c &&                       \
...
ld -nostdlib -m elf_i386 -T casm/link.ld                          \
  -o ../output/kernel.bin                                             \
